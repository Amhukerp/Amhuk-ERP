...
app.post("/api/shopify/order-lookup", async (req, res) => {
  try {
    ...
    const input = String(contact).toLowerCase().replace(/\s/g, "");
    const email = order.customer?.email?.toLowerCase() || "";
    const phone = String(order.customer?.phone || order.shippingAddress?.phone || "").replace(/\s/g, "");
    ...
    if (!matched) {
      return res.status(403).json({
        error: "Order ID and contact details do not match"
      });
    }
